...
const defaultDescription =
  "SoleCraft is a premium Pakistani footwear store for considered everyday pairs, traditional craft, and modern movement.";
...
export function Seo({ pathname, title, description, schema }: SeoProps) {
  useEffect(() => {
    const resolvedTitle =
      title ??
      (pathname === "/"
        ? "SoleCraft | Premium Pakistani Footwear"
        : pathname.startsWith("/shop")
          ? "Shop Footwear | SoleCraft Pakistan"
          : pathname.startsWith("/product/")
            ? "Footwear Details | SoleCraft Pakistan"
            : pathname.startsWith("/about")
              ? "Our Craft | SoleCraft Pakistan"
              : "SoleCraft Pakistan");
    ...
    setProperty(
      "og:type",
      schema?.["@type"] === "Product" ? "product" : "website"
    );
    ...
    let schemaTag = document.getElementById(
      "solecraft-store-schema"
    ) as HTMLScriptElement | null;
    if (!schemaTag) {
      schemaTag = document.createElement("script");
      schemaTag.type = "application/ld+json";
      schemaTag.id = "solecraft-store-schema";
