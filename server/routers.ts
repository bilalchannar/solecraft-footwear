...
export const appRouter = router({
  ...
  cart: router({
    ...
    add: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        addCartItem(ctx.user.id, input.variantId, input.quantity)
      ),
    updateQuantity: protectedProcedure
      .input(
        z.object({
          itemId: z.number().int().positive(),
          quantity: z.number().int().min(0).max(12),
        })
      )
      .mutation(({ ctx, input }) =>
        changeCartItemQuantity(ctx.user.id, input.itemId, input.quantity)
      ),
  }),
  wishlist: router({
    ...
    toggle: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        toggleWishlistItem(ctx.user.id, input.productId)
      ),
  }),
  account: router({
    profile: protectedProcedure.query(({ ctx }) =>
      getCustomerProfile(ctx.user.id)
    ),
    saveProfile: protectedProcedure
      ...
      .mutation(({ ctx, input }) => saveCustomerProfile(ctx.user.id, input)),
    addresses: protectedProcedure.query(({ ctx }) =>
    ...
    saveAddress: protectedProcedure
      ...
      .mutation(({ ctx, input }) =>
        saveAddress(ctx.user.id, input.address, input.addressId)
      ),
    orders: protectedProcedure.query(({ ctx }) =>
      listOrdersForUser(ctx.user.id)
    ),
    order: protectedProcedure
      .input(z.object({ publicId: z.string().uuid() }))
      .query(({ ctx, input }) => getOrderForUser(ctx.user.id, input.publicId)),
    returns: protectedProcedure.query(({ ctx }) =>
      listReturnsForUser(ctx.user.id)
    ),
  ...
  checkout: router({
    quote: protectedProcedure
      .input(z.object({ couponCode: z.string().trim().max(64).optional() }))
      .query(({ ctx, input }) => quoteCheckout(ctx.user.id, input.couponCode)),
  ...
  newsletter: router({
    subscribe: publicProcedure
      ...
      .mutation(({ ctx, input }) =>
        subscribeNewsletter(input.email, ctx.user?.id, input.source)
      ),
    unsubscribe: publicProcedure
      .input(z.object({ token: z.string().trim().min(16).max(96) }))
      .mutation(({ input }) => unsubscribeNewsletter(input.token)),
  }),
  reviews: router({
  ...
  admin: router({
    ...
    updateReturnStatus: staffProcedure
      .input(
        z.object({
      ...
      .mutation(({ ctx, input }) =>
        updateReturnStatus(input.returnId, input.status, ctx.user.id)
      ),
    updateReviewStatus: staffProcedure
      .input(
        z.object({
      ...
      .mutation(({ input }) =>
        updateReviewStatus(input.reviewId, input.status)
      ),
    ...
    saveCoupon: adminProcedure
      ...
      .mutation(({ input }) => saveCoupon(input, input.couponId)),
    saveDiscount: adminProcedure
      ...
      .mutation(({ input }) => saveDiscount(input, input.discountId)),
    saveHomepageSection: adminProcedure
      ...
      .mutation(({ input }) => saveHomepageSection(input)),
    saveBanner: adminProcedure
      ...
      .mutation(({ input }) => saveBanner(input)),
    saveSetting: adminProcedure
      ...
      .mutation(({ input }) => saveSiteSetting(input.key, input.value)),
    updateProduct: adminProcedure
      ...
      .mutation(({ input }) => {
        ...
        return updateProductCore(productId, product);
      }),
    archiveProduct: adminProcedure
      .input(z.object({ productId: z.number().int().positive() }))
      .mutation(({ input }) => archiveProduct(input.productId)),
  }),
});
