...
export const userRoleEnum = pgEnum("user_role", ["user", "staff", "admin", "super_admin"]);
export const productStatusEnum = pgEnum("product_status", ["draft", "active", "archived"]);
export const variantAvailabilityEnum = pgEnum("variant_availability", ["available", "out_of_stock", "discontinued"]);
export const couponKindEnum = pgEnum("coupon_kind", ["percentage", "fixed", "free_shipping"]);
export const discountTypeEnum = pgEnum("discount_type", ["product", "category", "storewide", "flash_sale", "buy_x_get_y", "first_order"]);
export const discountValueTypeEnum = pgEnum("discount_value_type", ["percentage", "fixed"]);
export const orderStatusEnum = pgEnum("order_status", ["pending", "confirmed", "processing", "packed", "shipped", "out_for_delivery", "delivered", "cancelled", "returned", "refunded"]);
export const paymentMethodEnum = pgEnum("payment_method", ["cod", "online"]);
export const orderPaymentStatusEnum = pgEnum("order_payment_status", ["pending", "paid", "failed", "cancelled", "refunded", "partially_refunded"]);
export const paymentRecordStatusEnum = pgEnum("payment_record_status", ["pending", "paid", "failed", "cancelled", "refunded", "partially_refunded"]);
...
export const checkoutStatusEnum = pgEnum("checkout_status", ["started", "completed", "failed"]);
export const inventoryAdjustmentKindEnum = pgEnum("inventory_adj_kind", ["receive", "sale", "return", "manual", "correction"]);
export const shipmentStatusEnum = pgEnum("shipment_status", ["pending", "label_created", "in_transit", "out_for_delivery", "delivered", "returned", "failed"]);
export const returnStatusEnum = pgEnum("return_status", ["requested", "approved", "rejected", "received", "cancelled"]);
export const returnRefundStatusEnum = pgEnum("return_refund_status", ["not_requested", "pending", "partial", "refunded", "failed"]);
...
export const categories = pgTable("categories", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("categories_parent_active_idx").on(table.parentId, table.active)]);
...
export const products = pgTable("products", {
  ...
  index("products_catalog_idx").on(table.categoryId, table.status, table.createdAt),
  index("products_discovery_idx").on(table.status, table.featured, table.isNew, table.bestSeller),
...
export const productVariants = pgTable("productVariants", {
  ...
  salePriceOverride: numeric("salePriceOverride", { precision: 12, scale: 2 }),
  imageUrl: text("imageUrl"),
  availability: variantAvailabilityEnum("availability").default("available").notNull(),
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [
  uniqueIndex("variants_product_size_color_uq").on(table.productId, table.size, table.color),
  index("variants_product_availability_idx").on(table.productId, table.availability),
]);

export const inventory = pgTable("inventory", {
  id: serial("id").primaryKey(),
  variantId: integer("variantId").notNull().unique().references(() => productVariants.id, { onDelete: "cascade" }),
  ...
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("inventory_stock_idx").on(table.stockOnHand, table.lowStockThreshold)]);

export const productImages = pgTable("productImages", {
  id: serial("id").primaryKey(),
  productId: integer("productId").notNull().references(() => products.id, { onDelete: "cascade" }),
  variantId: integer("variantId").references(() => productVariants.id, { onDelete: "set null" }),
  storageKey: varchar("storageKey", { length: 512 }),
  url: text("url").notNull(),
  altText: varchar("altText", { length: 255 }),
  sortOrder: integer("sortOrder").default(0).notNull(),
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("product_images_order_idx").on(table.productId, table.sortOrder)]);
...
export const cartItems = pgTable("cartItems", {
  ...
  variantId: integer("variantId").notNull().references(() => productVariants.id, { onDelete: "restrict" }),
...
export const wishlistItems = pgTable("wishlistItems", {
  wishlistId: integer("wishlistId").notNull().references(() => wishlists.id, { onDelete: "cascade" }),
  productId: integer("productId").notNull().references(() => products.id, { onDelete: "cascade" }),
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
...
export const coupons = pgTable("coupons", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("coupons_active_window_idx").on(table.active, table.startsAt, table.expiresAt)]);

export const couponUsage = pgTable("couponUsage", {
  id: serial("id").primaryKey(),
  couponId: integer("couponId").notNull().references(() => coupons.id, { onDelete: "restrict" }),
  ...
  usedAt: timestamp("usedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
...
export const discounts = pgTable("discounts", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("discount_active_window_idx").on(table.active, table.startsAt, table.expiresAt)]);

export const discountProducts = pgTable("discountProducts", {
  discountId: integer("discountId").notNull().references(() => discounts.id, { onDelete: "cascade" }),
  productId: integer("productId").notNull().references(() => products.id, { onDelete: "cascade" }),
}, table => [primaryKey({ columns: [table.discountId, table.productId] })]);

export const discountCategories = pgTable("discountCategories", {
  discountId: integer("discountId").notNull().references(() => discounts.id, { onDelete: "cascade" }),
...
export const orders = pgTable("orders", {
  ...
  userId: integer("userId").notNull().references(() => users.id, { onDelete: "restrict" }),
  addressId: integer("addressId").references(() => addresses.id, { onDelete: "set null" }),
  status: orderStatusEnum("status").default("pending").notNull(),
  paymentMethod: paymentMethodEnum("paymentMethod").notNull(),
  paymentStatus: orderPaymentStatusEnum("paymentStatus").default("pending").notNull(),
  subtotal: numeric("subtotal", { precision: 12, scale: 2 }).notNull(),
  productDiscount: numeric("productDiscount", { precision: 12, scale: 2 }).default("0").notNull(),
  couponCode: varchar("couponCode", { length: 64 }),
  couponDiscount: numeric("couponDiscount", { precision: 12, scale: 2 }).default("0").notNull(),
  shippingAmount: numeric("shippingAmount", { precision: 12, scale: 2 }).default("0").notNull(),
  ...
  placedAt: timestamp("placedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [
...
export const orderItems = pgTable("orderItems", {
  ...
  discountAmount: numeric("discountAmount", { precision: 12, scale: 2 }).default("0").notNull(),
  quantity: integer("quantity").notNull(),
...
export const reviews = pgTable("reviews", {
  ...
  productId: integer("productId").notNull().references(() => products.id, { onDelete: "cascade" }),
  userId: integer("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  orderItemId: integer("orderItemId").references(() => orderItems.id, { onDelete: "set null" }),
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("reviews_product_status_idx").on(table.productId, table.status)]);
...
export const banners = pgTable("banners", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("banners_placement_idx").on(table.placement, table.active, table.sortOrder)]);
...
export const analyticsEvents = pgTable("analyticsEvents", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("analytics_event_time_idx").on(table.eventType, table.createdAt)]);

export const checkoutRequests = pgTable("checkoutRequests", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull().references(() => users.id, { onDelete: "cascade" }),
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("checkout_requests_user_idx").on(table.userId, table.createdAt)]);
...
export const inventoryAdjustments = pgTable("inventoryAdjustments", {
  ...
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("inventory_adjustments_variant_idx").on(table.variantId, table.createdAt)]);
...
export const shipments = pgTable("shipments", {
  ...
  estimatedDeliveryAt: timestamp("estimatedDeliveryAt", { withTimezone: true, mode: "date" }),
...
export const returns = pgTable("returns", {
  id: serial("id").primaryKey(),
  publicId: varchar("publicId", { length: 36 }).notNull().unique(),
  orderId: integer("orderId").notNull().references(() => orders.id, { onDelete: "restrict" }),
  userId: integer("userId").notNull().references(() => users.id, { onDelete: "restrict" }),
  reason: varchar("reason", { length: 500 }).notNull(),
  status: returnStatusEnum("status").default("requested").notNull(),
  refundStatus: returnRefundStatusEnum("refundStatus").default("not_requested").notNull(),
  refundAmount: numeric("refundAmount", { precision: 12, scale: 2 }),
  restock: integer("restock").default(1).notNull(),
  createdAt: timestamp("createdAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
}, table => [index("returns_order_customer_idx").on(table.orderId, table.userId, table.createdAt)]);

export const returnItems = pgTable("returnItems", {
  id: serial("id").primaryKey(),
  returnId: integer("returnId").notNull().references(() => returns.id, { onDelete: "cascade" }),
  orderItemId: integer("orderItemId").notNull().references(() => orderItems.id, { onDelete: "restrict" }),
  quantity: integer("quantity").notNull(),
});
...
export const newsletterSubscribers = pgTable("newsletterSubscribers", {
  ...
  subscribedAt: timestamp("subscribedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
  unsubscribedAt: timestamp("unsubscribedAt", { withTimezone: true, mode: "date" }),
  updatedAt: timestamp("updatedAt", { withTimezone: true, mode: "date" }).defaultNow().notNull(),
});
